...
const getUsersReservaitons = async (userId) => {
  try {
    const SQL = `SELECT reservations.id, books.title, books.description, books.coveriamge, books.author FROM reservations JOIN books ON reservations.bookid = books.id AND userid=$1`;
